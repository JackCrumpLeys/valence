...
pub(super) fn derive_packet(item: TokenStream) -> Result<TokenStream> {
    ...
    let state = packet_attr
        .state
        .unwrap_or_else(|| parse_quote!(::valence_binary::PacketState::Play));
    ...
    let packet_id: Expr = match packet_attr.id {
        ...
        None => match syn::parse_str::<Ident>(&(suffix + &*name_str).to_shouty_snake_case()) {
            Ok(ident) => parse_quote!(::valence_generated::packet_id::#ident),
            Err(_) => {
    ...
    let side = if let Some(side_attr) = packet_attr.side {
        ...
        parse_quote!(::valence_binary::PacketSide::Clientbound)
    } else if name_str.to_lowercase().ends_with("c2s") {
        parse_quote!(::valence_binary::PacketSide::Serverbound)
    } else {
        return Err(Error::new(
            packet_attr.span,
    ...
    Ok(quote! {
        impl #impl_generics ::valence_binary::__private::Packet for #name #ty_generics
        #where_clause
        {
            const ID: i32 = #packet_id;
            const NAME: &'static str = #name_str;
            const SIDE: ::valence_binary::PacketSide = #side;
            const STATE: ::valence_binary::PacketState = #state;
        }
    })
}
